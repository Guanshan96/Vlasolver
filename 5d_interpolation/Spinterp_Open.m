function Si = Spinterp_Open(f, dx, dt, F, dim)
%----------------------------------------------------------
%   1-D Parallel cubic spline interpolation with open BC
%----------------------------------------------------------
...
%	support up to 5-D array and use open boundary
%	condition (The first derivative at boundary is 
...
un = ones(n,1)*dx;
A = spdiags([un, 4*un, un], [-1 0 1], n, n);
A(1) = 2*dx;A(end) = 2*dx;
A = full(A);A = A^-1;A = gpuArray(A);
